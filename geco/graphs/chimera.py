        ...
        for cur_m in range(m):
            for left_t in range(t):
                for right_t in range(t):
                    g.add_edge((cur_n, cur_m, left_t, 0),
                               (cur_n, cur_m, right_t, 1), weight=inter())

    # add horizontal inter-cell edges
    for cur_n in range(n):
        for cur_m in range(m - 1):
            for cur_t in range(t):
                g.add_edge((cur_n, cur_m, cur_t, 1),
                           (cur_n, cur_m + 1, cur_t, 1), weight=intra())

    # add vertical inter-cell edges
    for cur_n in range(n - 1):
        for cur_m in range(m):
            for cur_t in range(t):
                g.add_edge((cur_n, cur_m, cur_t, 0),
                           (cur_n + 1, cur_m, cur_t, 0), weight=intra())
...
def selby_c(m, seed=0) -> nx.Graph:
    ...
    def inter_w(): return np.random.randint(low=-10, high=10 + 1) / 10.0

    def intra_w(): return np.random.randint(low=-5, high=5 + 1) / 10.0
...
def _initialize_weights_chimera(chimera_graph, size, draw_inter_weight, draw_intra_weight):
    y, x, u, k = range(4)
...
@preserve_random_state
def dwave_chimera_graph(size, seed=0, draw_inter_weight=draw_inter_weight, draw_intra_weight=draw_intra_weight):
    np.random.seed(seed)
